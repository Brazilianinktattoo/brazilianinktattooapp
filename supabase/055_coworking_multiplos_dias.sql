-- Antes, coworking_passes.profile_id era unique — um visitante só podia ter
-- UM pass na vida inteira, então cada dia reservado exigia criar um login
-- novo do zero (foi assim que "Miguel Figini" virou 3 contas duplicadas,
-- uma por dia). Agora um mesmo profile pode ter vários passes (um por dia
-- reservado), reaproveitando o mesmo login/link — sem limite de dias.

alter table public.coworking_passes
  drop constraint coworking_passes_profile_id_key;

-- Com múltiplos passes por profile, "pega sempre o último criado" não
-- serve mais pra achar o acesso válido agora — troca por "prioriza o pass
-- cujo período contém o momento atual", com o mais recente como
-- desempate/fallback.
...
begin
  ...
  if collaborator_role = 'visitante' then
    ...
    order by
      (now() between starts_at and ends_at) desc,
      starts_at desc
    limit 1;
