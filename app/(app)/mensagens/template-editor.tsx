"use client";

import { useState, useTransition } from "react";
import { updateMessageTemplate } from "@/app/actions/mensagens";
import type { MessageTemplate, MessageTrigger } from "@/lib/types/database";

const LABELS: Record<MessageTrigger, string> = {
  aniversario: "Aniversário",
  pos_tattoo_1: "Pós-tattoo — 1 dia",
  pos_tattoo_7: "Pós-tattoo — 7 dias",
  pos_tattoo_15: "Pós-tattoo — 15 dias",
  pos_tattoo_30: "Pós-tattoo — 30 dias",
  pos_tattoo_60: "Pós-tattoo — 60 dias",
};

export function TemplateEditor({ template }: { template: MessageTemplate }) {
  const [body, setBody] = useState(template.body);
  const [active, setActive] = useState(template.active);
  const [pending, startTransition] = useTransition();
  const [saved, setSaved] = useState(false);

  const dirty = body !== template.body || active !== template.active;

  return (
    <div className="rounded-xl border border-neutral-800 bg-neutral-900/40 p-5">
      <div className="flex items-center justify-between gap-3">
        <h3 className="font-medium text-white">{LABELS[template.trigger]}</h3>
        <button
          type="button"
          disabled={pending}
          onClick={() => {
            const next = !active;
            setActive(next);
            setSaved(false);
          }}
          className={`shrink-0 rounded-full px-3 py-1 text-xs font-medium disabled:opacity-60 ${
            active
              ? "bg-green-500/15 text-green-400 hover:bg-green-500/25"
              : "bg-neutral-700/40 text-neutral-400 hover:bg-neutral-700/60"
          }`}
        >
          {active ? "Ativa" : "Inativa"}
        </button>
      </div>
      <textarea
        value={body}
        onChange={(e) => {
          setBody(e.target.value);
          setSaved(false);
        }}
        rows={3}
        className="mt-3 w-full rounded-lg border border-neutral-700 bg-neutral-900 px-3 py-2 text-sm text-neutral-100 outline-none focus:border-gold"
      />
      <p className="mt-1 text-xs text-neutral-500">
        Use {"{{nome}}"} pra inserir o primeiro nome do cliente.
      </p>
      <div className="mt-2 flex items-center gap-3">
        <button
          type="button"
          disabled={pending || !dirty}
          onClick={() =>
            startTransition(async () => {
              await updateMessageTemplate(template.trigger, body, active);
              setSaved(true);
            })
          }
          className="rounded-lg bg-gradient-to-b from-gold-strong to-gold shadow-[0_4px_14px_-4px_rgba(201,169,97,0.45)] px-4 py-2 text-sm font-medium text-neutral-950 transition hover:to-copper disabled:cursor-not-allowed disabled:opacity-60"
        >
          {pending ? "Salvando..." : "Salvar"}
        </button>
        {saved && !dirty && <span className="text-sm text-green-400">Salvo.</span>}
      </div>
    </div>
  );
}
