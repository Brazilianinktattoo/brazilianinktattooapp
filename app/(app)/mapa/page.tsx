...
import type { AppointmentWithRelations, Maca } from "@/lib/types/database";
...
export default async function MapaPage(props: PageProps<"/mapa">) {
  ...
  const [{ data: macas }, { data: appointments }] = await Promise.all([
    supabase
      .from("macas")
      .select("*")
      .eq("active", true)
      ...
      <div className="grid grid-cols-1 gap-4 overflow-x-auto sm:grid-cols-2 lg:grid-cols-5">
        {(macas ?? []).map((maca) => {
          ...
          return (
            ...
            >
              <h2 className="font-semibold text-white">{maca.label}</h2>

              {items.length === 0 ? (
                <p className="text-sm text-neutral-500">Livre o dia todo</p>
              ) : (
                      ...
                      <div key={appt.id} className="flex flex-col gap-2">
                        {prevEnd && gapMinutes > 0 && (
                          <div className="rounded-md border border-dashed border-neutral-700 px-2 py-1 text-center text-xs text-neutral-500">
                            Livre {formatTime(prevEnd)} – {formatTime(appt.starts_at)}
                          </div>
                        )}
        ...
        {(macas ?? []).length === 0 && (
          <p className="text-neutral-500">Nenhuma maca ativa cadastrada.</p>
        )}
      </div>
    </div>
  );
