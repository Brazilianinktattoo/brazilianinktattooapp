...
import {
  submitWalkinAnamnese,
  type WalkinAnamneseState,
...
export function WalkinAnamneseForm({
  professionals,
  consentText,
}: {
  professionals: Pick<Profile, "id" | "full_name" | "role">[];
  consentText: string;
}) {
  const [state, formAction, pending] = useActionState(submitWalkinAnamnese, initialState);
    ...
    return (
      <div className="flex flex-col items-center gap-3 rounded-xl border border-green-800 bg-green-500/10 p-8 text-center">
        ...
        <p className="text-green-200/80">
          Obrigado por preencher. Qualquer dúvida, fale com a recepção.
        </p>
  ...
  return (
    <form action={formAction} className="flex flex-col gap-6">
      <div className="flex flex-col gap-3 rounded-xl border border-gold-soft/30 bg-neutral-900/40 p-5">
        <h2 className="font-semibold text-white">Quem vai te atender?</h2>
        <select
        ...
        >
          ...
          {professionals.map((p) => (
            <option key={p.id} value={p.id}>
              {p.full_name || "Sem nome"} — {ROLE_LABEL[p.role] ?? p.role}
            </option>
          ))}
        </select>
      </div>
      ...
      <div className="flex flex-col gap-3 rounded-xl border border-neutral-800 bg-neutral-900/40 p-5">
        ...
        <div className="grid gap-3 sm:grid-cols-2">
          ...
          <Field id="phone" label="Telefone" required />
